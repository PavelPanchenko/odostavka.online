#!/bin/bash

# Скрипт для запуска всех сервисов кроме админки
# Включает: PostgreSQL, Redis, API, Client, Nginx

echo "🚀 Запуск всех сервисов приложения доставки еды..."

# Проверяем наличие файла окружения
if [ ! -f ".env" ]; then
    echo "⚠️  Файл .env не найден. Копируем из env.example..."
    cp env.example .env
    echo "✅ Файл .env создан из env.example"
    echo "📝 Отредактируйте .env файл при необходимости"
fi

# Останавливаем существующие контейнеры
echo "🛑 Останавливаем существующие контейнеры..."
docker compose -f docker-compose-all.yml down

# Собираем и запускаем все сервисы
echo "🔨 Собираем и запускаем все сервисы..."
docker compose -f docker-compose-all.yml up --build -d

# Ждем запуска базы данных
echo "⏳ Ждем запуска базы данных..."
sleep 15

# Проверяем статус контейнеров
echo "📊 Статус контейнеров:"
docker compose -f docker-compose-all.yml ps

echo ""
echo "✅ Все сервисы запущены!"
echo ""
echo "🌐 Доступные сервисы:"
echo "   - Веб-приложение: http://localhost"
echo "   - API: http://localhost/api"
echo "   - PostgreSQL: localhost:5432"
echo "   - Redis: localhost:6379"
echo ""
echo "📋 Полезные команды:"
echo "   - Просмотр логов: docker compose -f docker-compose-all.yml logs -f"
echo "   - Остановка: docker compose -f docker-compose-all.yml down"
echo "   - Перезапуск: docker compose -f docker-compose-all.yml restart"
echo ""
